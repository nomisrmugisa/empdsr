...
public interface SlaveAuditAuditRepository extends JpaRepository<audit_audit, String> {
	...
	@Query("select t.patient_factors FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<cfactor_table> findPatientFactorsByUuid(String uuid);

	@Query("select t.transport_factors FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<cfactor_table> findTransportFactorsByUuid(String uuid);

	@Query("select t.administrative_factors FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<cfactor_table> findAdministrativeFactorsByUuid(String uuid);

	@Query("select t.healthworker_factors FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<cfactor_table> findHealthworkerFactorsByUuid(String uuid);

	@Query("select t.document_factors FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<cfactor_table> findDocumentFactorsByUuid(String uuid);

	@Query("select t.maternal_conditions FROM slave_audit_audit t WHERE t.audit_uuid=?1")
	List<mcondition_table> findMaternalConditionsByUuid(String uuid);



	@Query("select t FROM slave_audit_audit t LEFT JOIN slave_audit_recommendation r ON(t.audit_uuid=r.audit_uuid) WHERE r.recommendation_uuid IS NULL")
	List<audit_audit> findByPendingRecommendation();

	@Query("select t.audit_icdpm, count(t.audit_icdpm) FROM slave_audit_audit t WHERE t.audit_death=1 "
			+ "GROUP BY t.audit_icdpm ORDER BY count(t.audit_icdpm) DESC")
	List<String[]> findByTopPMCodesStillBirth(Pageable pageable);

	@Query("select t.audit_icdpm, count(t.audit_icdpm) FROM slave_audit_audit t WHERE t.audit_death=1 AND YEAR(t.audit_cdate)=:cyear "
			+ "GROUP BY t.audit_icdpm ORDER BY count(t.audit_icdpm) DESC")
